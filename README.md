...
根据是否需要使用CAE图形界面，创建容器的方法略有不同，以下分别介绍。

#### 不使用CAE图形界面

如果不需要使用CAE图形界面，那么就非常简单。

通过abq2021镜像创建一个容器，起名为abq2021app，并把宿主机的/home目录挂载到容器的/home目录下，以便在容器中访问宿主机/home目录中的文件。
...
如果需要使用CAE图形界面，还需要在创建容器时加上参数 `--volume=/tmp/.X11-unix/:/tmp/.X11-unix/ --env="DISPLAY"` 。
...
其中，`--volume=/tmp/.X11-unix/:/tmp/.X11-unix/` 是将宿主机的X11 socket文件挂载到容器中，`--env="DISPLAY"` 是将宿主机的DISPLAY环境变量传递给容器。

**注意**：经过测试，如果创建容器时设置 `--net=host` ，MPI将无法正常工作，从而导致Abaqus/Explicit无法在mp_mode=domain模式下多核并行。MPI与容器的兼容性问题非常复杂（[参考文档](https://permedcoe.eu/wp-content/uploads/2021/10/mpi_in_containers.pdf)），这里不做讨论。

之后就可以启动CAE图形界面。**注意**，必须加上 `-mesa` 参数，否则会因为缺少依赖库而无法启动图形界面。这个参数的作用是关闭硬件加速，使用软件渲染。
...
首先要确保非root账户有执行docker命令的权限，确保非root账户加入了docker组。如果没有，可以用 `sudo usermod -aG docker $USER` 命令将当前用户加入docker组，然后注销重新登录即可执行docker命令。

要让docker中的软件以指定用户的权限读写宿主机的文件，就需要在容器中创建一个与宿主机用户具有相同UID和GID的用户，并且在exec时通过参数 `--user UID:GID` 指定这个UID和GID。如果容器中没有这个UID，还使用 `--user UID:GID` 强行指定UID和GID，那Abaqus许可证服务将无法正常运行。
...
之后**无论是什么用户**，只要有执行docker命令的权限，都可直接使用 `abq2021` 命令来运行Abaqus。
...
3. 选择安装产品的时候只选择1-5：
    ...
    **提示**：为了节省空间，我没有安装Abaqus Samples（约700MiB），这不影响使用，但运行 `abaqus verify -user_exp` 时会提示验证失败，因为验证user_exp所依赖的算例文件是随Abaqus Samples一起安装的。
...
上一步得到的临时Docker镜像已经几乎可以正常使用了，但在Abaqus调用MPI时，例如使用mp_mode=domain进行多核并行时，会报以下警告：
